...
var i18n = L.Class.singleton({
  ...
  label: function (host, plugin, pinst, dtype, dinst, user_label) {
    var label =
      user_label || "dt=%s/%di=%s".format(dtype || "(nil)", dinst || "(nil)");
    return subst(label, {
      host: host,
      plugin: plugin,
      pinst: pinst,
...
var colors = L.Class.singleton({
  fromString: function (s) {
    if (typeof s != "string" || !s.match(/^[0-9a-fA-F]{6}$/)) return null;
    return [
...
return baseclass.extend({
  ...
  load: function () {
    return Promise.all([
      L.resolveDefault(
        fs.list("/www" + L.resource("statistics/rrdtool/definitions")),
        []
      ),
      ...
      L.bind(function (data) {
        var definitions = data[0],
          hostname = data[1];
        this.opts.host =
          uci.get("luci_statistics", "collectd", "Hostname") || hostname;
        this.opts.timespan =
          uci.get("luci_statistics", "rrdtool", "default_timespan") || 3600;
        this.opts.width =
          uci.get("luci_statistics", "rrdtool", "image_width") || 600;
        this.opts.height =
          uci.get("luci_statistics", "rrdtool", "image_height") || 150;
        this.opts.rrdpath = (
          uci.get("luci_statistics", "collectd_rrdtool", "DataDir") ||
          "/tmp/rrd"
        ).replace(/\/$/, "");
        this.opts.rrasingle =
          uci.get("luci_statistics", "collectd_rrdtool", "RRASingle") == "1";
        this.opts.rramax =
          uci.get("luci_statistics", "collectd_rrdtool", "RRAMax") == "1";
        graphdefs = {};
        var tasks = [this.scan()];
        for (var i = 0; i < definitions.length; i++) {
          var m = definitions[i].name.match(/^(.+)\.js$/);
          if (definitions[i].type != "file" || m == null) continue;
          tasks.push(
            L.require("statistics.rrdtool.definitions." + m[1]).then(
              L.bind(
                function (name, def) {
                  graphdefs[name] = def;
                },
  ...
  ls: function () {
    ...
    return L.resolveDefault(fs.list(dir), []).then(function (entries) {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        tasks.push(
          L.resolveDefault(fs.list(dir + "/" + entries[i].name), []).then(
            L.bind(function (entries) {
              var tasks = [];
              for (var j = 0; j < entries.length; j++) {
                if (entries[j].type != "directory") continue;
                tasks.push(
                  L.resolveDefault(
                    fs.list(dir + "/" + this.name + "/" + entries[j].name),
                    []
                  ).then(
                    L.bind(function (entries) {
              ...
              return Promise.all(tasks).then(
                L.bind(function (entries) {
                  return Object.assign(this, { entries: entries });
                }, this)
              );
            }, entries[i])
          )
      ...
      L.bind(function (entries) {
        ...
        for (var i = 0; i < entries.length; i++) {
          ...
          for (var j = 0; j < entries[i].entries.length; j++) {
            ...
            var pluginName = m[1],
              pluginInstance = m[2] || "";
            rrdtree[hostInstance][pluginName] =
              rrdtree[hostInstance][pluginName] || {};
            rrdtree[hostInstance][pluginName][pluginInstance] =
              rrdtree[hostInstance][pluginName][pluginInstance] || {};
            for (var k = 0; k < entries[i].entries[j].entries.length; k++) {
              var m = entries[i].entries[j].entries[k].name.match(
                /^([^-]+)(?:-(.+))?\.rrd$/
              );
              if (!m) continue;
              var dataType = m[1],
                dataInstance = m[2] || "";
              rrdtree[hostInstance][pluginName][pluginInstance][dataType] =
                rrdtree[hostInstance][pluginName][pluginInstance][dataType] ||
                [];
              rrdtree[hostInstance][pluginName][pluginInstance][dataType].push(
                dataInstance
              );
            }
          }
    ...
    return Object.keys((rrdtree[hostInstance] || {})[pluginName] || {}).sort(
      function (a, b) {
        var x = a.match(/^(\d+)\b/),
          y = b.match(/^(\d+)\b/);
        if (!x != !y) return !x - !y;
        else if (x && y && x[0] != y[0]) return +x[0] - +y[0];
        else return a > b;
      }
    );
  },
  ...
  hasInstanceDetails: function (hostInstance, pluginName, pluginInstance) {
    var def = graphdefs[pluginName];
    if (!def || typeof def.rrdargs != "function") return false;
    var optlist = this._forcelol(
      def.rrdargs(this, hostInstance, pluginName, pluginInstance, null, false)
    );
    for (var i = 0; i < optlist.length; i++) if (optlist[i].detail) return true;
    return false;
  },
  ...
  mkrrdpath: function () {
    return "%s/%s.rrd"
      .format(this.opts.rrdpath, this._mkpath.apply(this, arguments))
      .replace(/[\\:]/g, "\\$&");
  },
  ...
  _rrdtool: function (def, rrd, timespan, width, height, cache) {
    var cmdline = [
      ...
      "PNG",
      "-s",
      "NOW-%s".format(timespan || this.opts.timespan),
      "-e",
      "NOW-15",
      "-w",
    ...
    for (var i = 0; i < def.length; i++) {
      ...
      if (rrd) opt = opt.replace(/\{file\}/g, rrd);
      cmdline.push(opt);
    }
    if (L.isObject(cache)) {
  ...
  _generic: function (opts, host, plugin, plugin_instance, dtype, index) {
    ...
    function __def(source) {
      ...
      _args.push("CDEF:%s_nnl=%s_avg,UN,0,%s_avg,IF".format(inst, inst, inst));
    }
    function __cdef(source) {
      var prev;
      if (source.flip) prev = _stack_neg[_stack_neg.length - 1];
      else prev = _stack_pos[_stack_pos.length - 1];
      if (prev == null || source.overlay) {
        ...
        if (gopts.rrasingle || !gopts.rramax)
          _args.push(
            "CDEF:%s_stk=%s_nnl,%s_stk,+".format(
              source.sname,
              source.sname,
              prev
            ),
            "CDEF:%s_plot=%s_avg,%s_stk,+".format(
              source.sname,
              source.sname,
              prev
            )
          ...
          _args.push(
            ...
            "CDEF:%s_plot=%s_max,%s_stk,+".format(
              source.sname,
              source.sname,
              prev
            )
      ...
      if (source.flip) {
        _args.push(
          "CDEF:%s_neg=%s_plot,-1,*".format(source.sname, source.sname)
        );
        if (!source.overlay) _stack_neg.push(source.sname);
      } else if (!source.overlay) {
        _stack_pos.push(source.sname);
      }
      if (source.total)
        _args.push(
          "CDEF:%s_avg_sample=%s_avg,UN,0,%s_avg,IF,sample_len,*".format(
            source.sname,
            source.sname,
            source.sname
          ),
          "CDEF:%s_avg_sum=PREV,UN,0,PREV,IF,%s_avg_sample,+".format(
            source.sname,
            source.sname,
            source.sname
          )
    ...
    function __line(source) {
      ...
      if (typeof source.color == "string") {
        ...
        typeof opts.colors[source.name.replace(/\W/g, "_")] == "string"
      ) {
    ...
    function __gprint(source) {
      ...
      if (!gopts.rrasingle)
        _args.push(
          "GPRINT:%s_min:MIN:\tMin\\: %s".format(source.sname, numfmt)
        );
      if (!source.noavg)
        _args.push(
          "GPRINT:%s_avg:AVERAGE:\tAvg\\: %s".format(source.sname, numfmt)
        );
      if (!gopts.rrasingle)
        _args.push(
          "GPRINT:%s_max:MAX:\tMax\\: %s".format(source.sname, numfmt)
        );
      if (source.total)
        _args.push(
          "GPRINT:%s_avg_sum:LAST:(ca. %s Total)\\l".format(
            source.sname,
            totfmt
          )
        );
        ...
        _args.push(
          "GPRINT:%s_avg:LAST:\tLast\\: %s\\l".format(source.sname, numfmt)
        );
    }
    ...
    for (var i = 0; i < data_types.length; i++) {
      ...
      if (!opts.per_instance) {
        if (
          L.isObject(opts.data.instances) &&
          Array.isArray(opts.data.instances[data_types[i]])
        )
          data_instances = opts.data.instances[data_types[i]];
        else
          data_instances = this.dataInstances(
            host,
            plugin,
            plugin_instance,
            data_types[i]
      ...
      for (var j = 0; j < data_instances.length; j++) {
        ...
        if (data_instances[j].length) dname += "_" + data_instances[j];
        ...
        for (var k = 0; k < data_sources.length; k++) {
          var dsname =
              data_types[i] +
              "_" +
              data_instances[j].replace(/\W/g, "_") +
              "_" +
              data_sources[k],
          ...
          var source = {
            ...
            weight:
              dopts.weight ||
              (dopts.negweight ? -+data_instances[j] : null) ||
              (dopts.posweight ? +data_instances[j] : null) ||
              null,
            ds: data_sources[k],
            type: data_types[i],
          ...
          if (source.total) _has_totals = true;
        }
      }
    }
    var instances = [""];
    if (opts.per_instance)
      instances = this.dataInstances(
        host,
        plugin,
        plugin_instance,
        _sources[0].type
    ...
    for (var i = 0; i < instances.length; i++) {
      ...
      if (opts.y_max) _args.push("-u", String(opts.y_max));
      if (opts.y_min) _args.push("-l", String(opts.y_min));
      if (opts.rigid) _args.push("-r");
      if (opts.units_exponent) _args.push("-X", String(opts.units_exponent));
      if (opts.alt_autoscale) _args.push("-A");
      if (opts.alt_autoscale_max) _args.push("-M");
      ...
      __cdef_totals();
      for (var j = _sources.length - 1; j >= 0; j--) __cdef(_sources[j]);
      for (var j = 0; j < _sources.length; j++) {
        __line(_sources[j]);
        __gprint(_sources[j]);
      }
  ...
  render: function (
    ...
    if (def && typeof def.rrdargs == "function") {
      var _images = [];
      var optlist = this._forcelol(
        def.rrdargs(this, hostname, plugin, plugin_instance, null, is_index)
      );
      for (var i = 0; i < optlist.length; i++) {
        var opt = optlist[i];
        if (!is_index || !opt.detail) {
          _images[i] = [];
          var diagrams = this._generic(
            opt,
            hostname,
            plugin,
            plugin_instance,
      ...
      for (var y = 0; y < _images[0].length; y++)
        for (var x = 0; x < _images.length; x++) pngs.push(_images[x][y]);
    }
