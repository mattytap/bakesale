...
nft add set inet vpn_bypass_analysis tcp_sin_bin '{ type ipv4_addr; flags interval,timeout; counter; }'
nft add set inet vpn_bypass_analysis tcp_archive '{ type ipv4_addr; flags dynamic,timeout; counter; }'
...
nft add rule inet vpn_bypass_analysis prerouting 'ip daddr 192.168.240.101 tcp sport 443 ip saddr @tcp_sin_bin counter'
nft add rule inet vpn_bypass_analysis prerouting 'ip daddr 192.168.240.101 tcp sport 443 ip saddr != @vpn_bypass ip saddr != @tcp_443_net ip saddr != @tcp_sin_bin counter update @tcp_443 { ip saddr }'
