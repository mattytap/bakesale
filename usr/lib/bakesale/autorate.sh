...
# autorate automatically adjusts CAKE bandwidth(s)
...
# each autorate instance must be configured using a corresponding config file 

# Based on original Project homepage: https://github.com/lynxthecat/cake-autorate
# Based on original Licence details:  https://github.com/lynxthecat/cake-autorate/blob/master/LICENCE.md

# Author and maintainer: lynxthecat
# Contributors:  rany2; moeller0; richb-hanover

autorate_version="3.1.0-MATTYTAP"

## autorate uses multiple asynchronous processes including:
## main - main process
...
{	
	...
	log_msg "INFO" "Stopping autorate with PID: ${BASHPID} and config: ${config_path}"
	...
	rmdir /var/run/autorate 2>/dev/null
	...
	log_msg "SYSLOG" "Stopped autorate with PID: ${BASHPID} and config: ${config_path}"
	...
	case ${type} in
		...
		DEBUG)
			...
			((log_DEBUG_messages_to_syslog)) && ((use_logger)) && \
				logger -t "autorate.${instance_id}" "${type}: ${log_timestamp} ${msg}"
			;;
		...
		SYSLOG)
			((use_logger)) && \
				logger -t "autorate.${instance_id}" "INFO: ${log_timestamp} ${msg}"
			;;
...
maintain_log_file()
{
	...
	do
		...
		# 	read log_file_size_bytes< <(du -b ${log_file_path}/autorate.log)
...
intercept_stderr() 
{
	# send stderr to log_msg and exit autorate
...
log_file_path=/var/log/autorate.log

# *** WARNING: take great care if attempting to alter the run_path! ***
# *** autorate issues mkdir -p ${run_path} and rm -r ${run_path} on exit. ***
run_path=/var/run/autorate/

# autorate first argument is config file path
if [[ -n "${1-}" ]]
then
	config_path="${1}"
else
...
then
	...
	run_path="/var/run/autorate/${instance_id}"
else
...
then
	...
	then
		...
		log_file_path=/var/log/autorate${instance_id:+.${instance_id}}.log
		log_msg "ERROR" "Log file path override: '${broken_log_file_path_override}' does not exist. Exiting now."
		exit
	fi
	log_file_path=${log_file_path_override}/autorate${instance_id:+.${instance_id}}.log
else
	log_file_path=/var/log/autorate${instance_id:+.${instance_id}}.log
fi
...
# intercept_stderr sends stderr to log_msg and exits autorate
exec {original_stderr_fd}>&2 2> >(intercept_stderr)

proc_pids['intercept_stderr']=${!}

log_msg "SYSLOG" "Starting autorate with PID: ${BASHPID} and config: ${config_path}"
...
if ((debug))
then
	log_msg "DEBUG" "autorate version: ${autorate_version}"
...
log_msg "INFO" "Started autorate with PID: ${BASHPID} and config: ${config_path}"
