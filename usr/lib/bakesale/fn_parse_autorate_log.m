function [ ] = fn_parse_autorate_log( log_FQN, plot_FQN, x_range_sec, selected_reflector_subset )
	...
	% octave -qf --eval 'fn_parse_autorate_log("./SCRATCH/autorate.log.20221001_1724_RRUL_fast.com.log", "./outpug.tif", [10, 500], {"1.1.1.1"})'
	...
	% by default the code will open a file selection dialog which should be used to select a autorate log file.
	...
	try
		...
		if ~exist('log_FQN', 'var') || isempty(log_FQN)
			log_FQN = [];
			% for debugging
			%log_FQN = "./SCRATCH/autorate_2022-10-29_23_29_45.log";
		else
...
%			end
		...
		if ismember('timecourse', plot_list);
			...
			autorate_fh = figure('Name', 'autorate log: rate & delay timecourses', 'visible', figure_visibility_string);
		...
		disp('Please report any issue to https://github.com/mattytap/bakesale/issues and consider sharing the log file that revealed the problem.');
	end_try_catch
...
function [ autorate_CDF_fh ] = fn_plot_CDF_by_measure_and_load_condition( distribution_string, figure_opts, data_struct, cumulative_range_percent, xlabel_string, ylabel_string, cur_plot_FQN )
	...
	autorate_CDF_fh = figure('Name', 'autorate log: delay CDFs', 'visible', figure_opts.figure_visibility_string);
